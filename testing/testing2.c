...
int compareErrors(const void *a, const void *b) {
    ...
    const struct Error *errorA = (const struct Error *)a;
    const struct Error *errorB = (const struct Error *)b;

    /*/ Compare error codes*/
    if (errorA->line < errorB->line) {
        return -1; /*/ `errorA` comes before `errorB`*/
    } else if (errorA->line > errorB->line) {
...
void implementSET(char **cleaned, int cleaned_size) {
    ...
    labels = (struct Label *)malloc(labels_capacity * sizeof(struct Label));
    int i;
        ...
        for ( j = 0; j < (int)strlen(line); j++) {
            ...
            if (line[j] == ':') {
                ...
                if (strlen(line) > j + 5 && strncmp(line + j + 2, "SET", 3) == 0) {
                    ...
                    if (label_index >= 0 && abs(labels[label_index].line) == i) {
...
void addOrUpdateLabel(const char *label, int index) {
    ...
    if (label_index >= 0) {
        labels[label_index].line = index;
    } else {
        if (labels_size >= labels_capacity) {
            labels_capacity *= 2;
            labels = realloc(labels, labels_capacity * sizeof(struct Label));
        }
        strncpy(labels[labels_size].name, label, MAX_LABEL_SIZE - 1);
        labels[labels_size].line = index;
        labels_size++;
    }
}
...
void firstPass() {
    ...
    data = (struct Data*)realloc(data, cleaned_size * sizeof(struct Data));
    programCounter = (int*)realloc(programCounter, cleaned_size * sizeof(int));
